%% w8a_SVM_batch
% This script runs a SVM on a sliding window over the timecourse of trials in the Main_experiment tasks. The SVM
...
for Session = 1:length(neural_dir) %for all sessions for this monkey
    ...
    window_size = 0.2; %Size of the integration window in seconds
    Time_points = 300:100:4700; %Time points over which the SVM will be calculated (sliding window)
    epoch_num = 0;
...
line([1500 1500], [0 100], 'Color', 'k') % mark cue onset
line([2500 2500], [0 100], 'Color', 'k') % mark cue offset
line([3000 3000], [0 100], 'Color', 'k', 'LineStyle', '--') % mark delay axis break
line([3500 3500], [0 100], 'Color', 'k') % mark target onset
line([3750 3750], [0 100], 'Color', 'k', 'LineStyle', '--') % mark reaction time axis break
line([4000 4000], [0 100], 'Color', 'k') % mark response onset

SVM_matrix = SVM_results*100;

%Select Krieger first implant sessions
%SVM_matrix = SVM_matrix([2 3 4 7 9 10 12 13 14], :);
%Select Krieger second implant sessions
%SVM_matrix = SVM_matrix([1 5 6 8 11], :);
