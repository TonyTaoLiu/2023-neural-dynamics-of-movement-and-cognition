...
% This is a batch implementation of w8a_Load_Organize_Data_script.m
...
% Created by Testard 05/2017

function [SpikeData, Timestamp_words, Words, BHV, head_position, words_per_trials, block, Waves, task] = w8a_Load_Organize_Data_batch(PathName, session_name, ifeyedata)

cd([PathName '/' session_name])
load(['BHV_' session_name '.mat']) %Load BHV
disp(' '); disp(['Loaded: ' 'BHV_' session_name '.mat']); disp(' ')
try
load(['NEVSorted_' session_name '.mat']) %Load NEV 
catch
    load(['NEV_' session_name '.mat'])
    warning('This session is not spike sorted!')
end
disp(' '); disp(['Loaded: ' 'NEVSorted_' session_name '.mat']); disp(' ')
tic
...
clearvars -except SpikeData Words Timestamp_words BHV ifeyedata PathName FileName session_name Waves
...
clearvars -except SpikeData Timestamp_words Words BHV ifeyedata PathName FileName session_name Waves
...
if ifeyedata == 1 && (exist(['EDF_' session_name '.asc'], 'file') || exist([session_name '.asc'], 'file')) % if you want to analyse eye data and there is actually eye data.
    
    cd([PathName '/' session_name])
    if exist(['EDF_' session_name '.txt'], 'file') || exist([session_name '.txt'], 'file') % If you already created that big corrected txt file
    else
        fid1 = fopen([session_name '.asc'],'r');
        fid2 = fopen([session_name '.txt'],'w');
    ...
    disp(' '); disp(['Loaded: EDF_' session_name '.txt']); disp(' ')
    ...
    if (EDF_TTL_times(end) - EDF_TTL_times(1)) - (BR_TTL_times(end) - BR_TTL_times(1)) <= 16 %msec
    else
    ...
    clearvars -except AnalogData words_per_trials SpikeData Timestamp_words Words BHV Eye_Data BR_TTL_times EDF_TTL_times PathName FileName session_name Waves
    ...
    clearvars -except AnalogData words_per_trials SpikeData Timestamp_words Words BHV head_position PathName FileName session_name Waves
    ...
    if mean_performance_delay < 0.5
        warning(['Only ' num2str(mean_performance_delay*100) '% of eye tracking is valid during the delay period.'])
    else
    end
    ...
    for i = 1:length(BHV.TrialError) %For each trial
        ...
        if sum(abs(BReyeX - BHVeyeX)/length(BHVeyeX)) > 600  % if the mean difference between the two curves is more than 500mv, output an error message. Sometimes the X data going to BR analog input doesn't get back to 0, creating a gap here.
...
total_neuron = sum(structfun(@numel,SpikeData)-1);
...
%% Select appropriate blocks with session:
if strfind(BHV.ConditionsFile, '35.All_new_associations') %If analyzing main task
    task = 1;
    if session_name == 'K200117'
        block(1) = 1;
        block(2) = 2;
    elseif session_name == 'K220117'
        block(1) = 10;
        block(2) = 12;
    elseif session_name == 'K230117'
        block(1) = 14;
        block(2) = 15;
    elseif session_name == 'K260117'
        block(1) = 5;
        block(2) = 1;
    elseif session_name == 'K300117'
        block(1) = 20;
        block(2) = 1;
    elseif session_name == 'K310117'
        block(1) = 27;
        block(2) = 1;
    elseif session_name == 'K020217'
        block(1) = 29;
        block(2) = 30;
    elseif session_name == 'K040217'
        block(1) = 37;
        block(2) = 38;
    elseif session_name == 'K050217'
        block(1) = 39;
        block(2) = 1;
    elseif session_name == 'K070217'
        block(1) = 25;
        block(2) = 1;
    elseif session_name == 'K190418'
        block(1) = 10;
        block(2) = 1;
    elseif session_name == 'K200418'
        block(1) = 27;
        block(2) = 29;
    elseif session_name == 'K230418'
        block(1) = 16;
        block(2) = 19;
    elseif session_name == 'K240418'
        block(1) = 16;
        block(2) = 19;
    elseif session_name == 'K250418'
        block(1) = 27;
        block(2) = 29;
    elseif session_name == 'K300418'
        block(1) = 10;
        block(2) = 1;
    elseif session_name == 'K010518'
        block(1) = 37;
        block(2) = 38;
    elseif session_name == 'K030518'
        block(1) = 37;
        block(2) = 38;
    elseif session_name == 'K140518'
        block(1) = 30;
        block(2) = 31;
    elseif session_name == 'K190618'
        block(1) = 10;
        block(2) = 1;
    elseif session_name == 'B250817'
        block(1) = 9;
        block(2) = 1;
    elseif session_name == 'B290817'
        block(1) = 13;
        block(2) = 1;
    elseif session_name == 'B020917'
        block(1) = 24;
        block(2) = 25;
    elseif session_name == 'B030917'
        block(1) = 36;
        block(2) = 37;
    elseif session_name == 'B040917'
        block(1) = 8;
        block(2) = 11;
    elseif session_name == 'B070917'
        block(1) = 1;
        block(2) = 6;
    elseif session_name == 'B080917'
        block(1) = 1;
        block(2) = 10;
    elseif session_name == 'B090917'
        block(1) = 20;
        block(2) = 23;
    elseif session_name == 'B110917'
        block(1) = 7;
        block(2) = 14;
    elseif session_name == 'B140917'
        block(1) = 1;
        block(2) = 2;
    elseif session_name == 'L080318'
        block(1) = 10;
        block(2) = 1;
    elseif session_name == 'L090318'
        block(1) = 10;
        block(2) = 1;
    elseif session_name == 'L120318'
        block(1) = 38;
        block(2) = 37;
    elseif session_name == 'L160318'
        block(1) = 38;
        block(2) = 37;
    elseif session_name == 'L180318'
        block(1) = 23;
        block(2) = 1;
    elseif session_name == 'L210318'
        block(1) = 23;
        block(2) = 1;
    elseif session_name == 'L220318'
        block(1) = 7;
        block(2) = 8;
    elseif session_name == 'L230318'
        block(1) = 1;
        block(2) = 27;
    elseif session_name == 'L240318'
        block(1) = 25;
        block(2) = 1;
    elseif session_name == 'L250318'
        block(1) = 6;
        block(2) = 1;
    elseif session_name == 'L260318'
        block(1) = 14;
        block(2) = 15;
    elseif session_name == 'L270318'
        block(1) = 36;
        block(2) = 27;
    elseif session_name == 'L030418'
        block(1) = 24;
        block(2) = 20;
    elseif session_name == 'L050418'
        block(1) = 23;
        block(2) = 1;
    elseif session_name == 'L170418'
        block(1) = 14;
        block(2) = 15;
    elseif session_name == 'L180418'
        block(1) = 36;
        block(2) = 27;
    elseif session_name == 'L190418'
        block(1) = 36;
        block(2) = 27;
    elseif session_name == 'K130618'
        block(1) = 50;
        block(2) = 51;
    elseif session_name == 'K080217'
        block(1) = 50;
        block(2) = 1;
    elseif session_name == 'K210618'
        block(1) = 64;
        block(2) = 64;
    else
        error('No blocks are defined for this session. Please check session log.')
    end
    
elseif strfind(BHV.ConditionsFile, 'fMRI') % If analyzing VDT
    task = 2;
    block(1) = 1;
    block(2) = 2;
    
elseif strfind(BHV.ConditionsFile, '36.Interleaved') % If analyzing Interleaved cue sessions
    task = 3;
    block(1) = 1;
    block(2) = 2;
    
elseif strfind(BHV.ConditionsFile, 'Delayed') % If analyzing DMS non-spatial sessions
    task = 4;
    block(1) = 1;
    
elseif strfind(BHV.ConditionsFile, '02.Spatial') % If analyzing spatial working memory sessions
    task = 5;
    block(1) = 1;
    if strfind(BHV.ConditionsFile, '_simplified') % If analyzing spatial WM with 4 targets
        task = 8;
    end
    
elseif strfind(BHV.ConditionsFile, '4tar') % If analyzing spatial WM with 4 targets and red circle distractor
    task = 6;
    block(1) = 1;
    
elseif strfind(BHV.ConditionsFile, 'Biased') % If analyzing biased CAT with 2 targets
    task = 7;
    block(1) = 1;
    
elseif strfind(BHV.ConditionsFile, '38.Comparing_Selecting_Targe') % If analyzing the single session in Krieger where only one target was presented (no distractor) on half the trials
    task = 8;
    block(1) = 1;    
    block(2) = 2;
...
clearvars -except SpikeData Timestamp_words Words BHV head_position words_per_trials block Waves task
