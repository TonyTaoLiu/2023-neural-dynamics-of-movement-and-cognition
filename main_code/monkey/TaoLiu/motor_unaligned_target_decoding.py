...
for ii in range(1):
    ...
    for id1,id2,testList in pairIndex_uni:
        ...
        AllData = dt.get_data_array([allDFs[id1]]+[allDFs[id2]]+[allDFs[testid] for testid in testList], defs.prep_epoch, area=defs.areas[2], model=defs.n_components)
        ...
        # resizing
        X1 = AllData1.reshape((-1, n_time1 * n_comp))
        ...
        X1_train, Y1_train = X1[trial_index1, :], AllTar1[trial_index1]
        ...
        for testId, _ in enumerate(testList):
            ...
            X2 = X2.reshape((-1, n_time1 * n_comp))
            X3 = X3.reshape((-1, n_time1 * n_comp))
            AllTar1 = np.repeat(np.arange(defs.n_targets), n_trial1)

            rng.shuffle(trial_index1)
            X2_test, Y2_test = X2[trial_index1, :], AllTar1[trial_index1]
            X3_test, Y3_test = X3[trial_index1, :], AllTar1[trial_index1]

            # test the decoder
            scores.append(classifier1.score(X2_test, Y2_test))
            scores.append(classifier1.score(X3_test, Y3_test))
...
warnings.filterwarnings("default")
# cahnge with your own saving path
np.save('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu/result/motor_M1_cls_scores_set_unaligned_nohist.npy',cls_scores_set)
file = open('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu/result/motor_M1_cls_scores_set_unaligned_nohist.txt','w')
