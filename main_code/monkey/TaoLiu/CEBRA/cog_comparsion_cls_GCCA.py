...
cls_scores = []
output_dim = 30 # dimension of latent embeddings
...
for id1 in pairIndex_uni[0]:
    for id2 in pairIndex_uni[1]:
        for id3 in pairIndex_uni[2]:
            ...
            X1 = AllData_train[0].reshape([-1, AllData[0].shape[-1]])
            X2 = AllData_train[1].reshape([-1, AllData[1].shape[-1]])
            X3 = AllData_train[2].reshape([-1, AllData[2].shape[-1]])
            ...
            X1_test_aligned = X_test1 @ gcca.weights[0]
            X2_test_aligned = X_test2 @ gcca.weights[1]
            X3_test_aligned = X_test3 @ gcca.weights[2]
            ...
            for subj in range(AllData.shape[0]):
                ...
                for iii in range(index_tmp.shape[0]): # test on rest subjects
                    ...
                    AllTar1 = np.repeat(np.arange(target_num), n_trial2) # obtain test labels
...
with open('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu\CEBRA/result/'
          'cog_Prefrontal8A_cls_scores_cue_pca30_aligned_CEBRA_comparsion_GCCA.pickle', 'wb') as f:
    pickle.dump(cls_scores, f)

file = open('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu/CEBRA/result/'
            'cog_Prefrontal8A_cls_scores_cue_pca30_aligned_CEBRA_comparsion_GCCA.txt','w')
