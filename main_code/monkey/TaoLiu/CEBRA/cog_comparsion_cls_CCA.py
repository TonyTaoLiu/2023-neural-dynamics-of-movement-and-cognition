    ...
    for id2 in pairIndex_uni[1]:
        for id3 in pairIndex_uni[2]:
            ...
            for subj in range(AllData.shape[0]):
                ...
                X1 = AllData_train[subj].reshape([-1, AllData[0].shape[-1]])
                X1_test = AllData_test[subj].reshape([-1,AllData[0].shape[-1]])
                for iii in range(index_tmp.shape[0]): # test on rest subjects
                    X2 = AllData_train[index_tmp[iii]].reshape([-1, AllData[1].shape[-1]])
                    X2_test = AllData_test[index_tmp[iii]].reshape([-1, AllData[1].shape[-1]])

                    A, B, *_ = dt.canoncorr(X1, X2, fullReturn=True) # align across 2 subjects
                    X1_aligned = X1_test @ A
                    X1_aligned = X1_aligned.reshape((-1, n_time1 * n_comp))
                    X2_aligned = X2_test @ B
...
with open('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu\CEBRA/result/'
          'cog_Prefrontal8A_cls_scores_cue_pca30_aligned_CEBRA_comparsion_CCA.pickle', 'wb') as f:
    pickle.dump(cls_scores, f)

file = open('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu/CEBRA/result/'
            'cog_Prefrontal8A_cls_scores_cue_pca30_aligned_CEBRA_comparsion_CCA.txt','w')
