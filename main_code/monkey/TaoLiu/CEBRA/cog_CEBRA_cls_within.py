...
for num,subj_id in enumerate(pairIndex_uni):
    ...
    for ii, X in enumerate(test_data):
        multi_embeddings.append(multi_cebra_model.transform(X, session_id=ii)) # transform test set

    latent_dynamics_full.append((subj_id[0],subj_id[1],subj_id[2],multi_embeddings))

    AllDyna = np.array([multi_embeddings[ii].reshape((n_targets, n_trial2, n_time1, output_dim)) for ii in
                        range(len(multi_embeddings))])
...
with open('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu\CEBRA/result'
          '\cognitive/cog_Prefrontal8A_latent_dynamics_cue_pca30_within_full.pickle', 'wb') as f:
    pickle.dump(latent_dynamics_full, f)

with open('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu\CEBRA/result/'
          'cog_Prefrontal8A_cls_scores_cue_pca30_within_CEBRA.pickle', 'wb') as f:
    pickle.dump(cls_scores, f)

file = open('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu/CEBRA/result/'
            'cog_Prefrontal8A_cls_scores_cue_pca30_within_CEBRA.txt','w')
