...
# form session pairs for alignment
pairIndex_uni = []
...
for ii in range(1):
    ...
    for id1,id2,testList in pairIndex_uni:
        # change to exec_epoch for comparison with CEBRA
        # change the defs.n_components to 10 for center-out task
        AllData = dt.get_data_array([allDFs[id1]]+[allDFs[id2]]+[allDFs[testid] for testid in testList],
                                    defs.prep_epoch, area=defs.areas[2], model=defs.n_components) # perform pca
        ...
        for testId, _ in enumerate(testList):
            ...
            A, B, *_ = dt.canoncorr(X1, X2, fullReturn=True) # align across 2 sessions
            ...
            scores.append(classifier1.score(X2_test, Y2_test)) # test on one resting monkey

            A, B, *_ = dt.canoncorr(X1, X3, fullReturn=True)
            X1_aligned = X1 @ A
            X1_aligned = X1_aligned.reshape((-1, n_time1 * n_comp))
            X3_aligned = X3 @ B
            X3_aligned = X3_aligned.reshape((-1, n_time1 * n_comp))
            AllTar1 = np.repeat(np.arange(defs.n_targets), n_trial1)
            ...
            scores.append(classifier2.score(X3_test, Y3_test)) # test on another resting monkey
...
# np.save('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu/result/motor_M1_cls_scores_set_withinsub_CEBRA_compare_CCA.npy',cls_scores_set)
# file = open('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu/result/motor_M1_cls_scores_set_withinsub_CEBRA_compare_CCA.txt','w')
# for fp in cls_scores_set:
#     file.write(str(fp))
#     file.write('\n')
# file.close()

np.save('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu/result/motor_M1_cls_scores_set_withinsub_prep_nohist_2subj.npy',cls_scores_set)
file = open('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu/result/motor_M1_cls_scores_set_withinsub_prep_nohist_2subj.txt','w')
for fp in cls_scores_set:
    file.write(str(fp))
    file.write('\n')
file.close()