...
for subj in pairIndex_uni:
    ...
    A1, B1, r1, *_ = dt.canoncorr(data1, data2, fullReturn=True) # align across 2 sessions
    A2, B2, r2, *_ = dt.canoncorr(data1, data3, fullReturn=True)
    A3, B3, r3, *_ = dt.canoncorr(data2, data3, fullReturn=True)
    CCA_score.append((subj[0],subj[1],subj[2],[r1,r2,r3])) # collect CCA scores (canonical correlations)
    ...
    alltar_latent_dynamics.append((subj[0], subj[1], subj[2], [[ex0_all1, ex1_all1], [ex0_all2, ex2_all1], [ex1_all2, ex2_all2]]))
    for tar in range(AllData.shape[1]):
        ...
        ex1_2 = ex1 @ U4 @ Vh4
        ex2_2 = ex2 @ U5 @ Vh5
        latent_dynamics.append((subj[0], subj[1], subj[2], tar, [[ex0_1,ex1_1], [ex0_2,ex2_1], [ex1_2,ex2_2]]))
    print('pair '+str(num)+' done')
    num += 1
# change with your own saving path
np.save('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu/result/cog_prefrontal8A_cue_latent_dynamics_withinsub_0.03_30_drift_2subj.npy',latent_dynamics)
np.save('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu/result/cog_prefrontal8A_alltar_cue_latent_dynamics_withinsub_0.03_30_drift_2subj.npy',alltar_latent_dynamics)
np.save('E:\MSc Project\code/2022-preserved-dynamics-main\monkey\TaoLiu/result/cog_prefrontal8A_cue_CCA_score_withinsub_0.03_30_drift_2subj.npy',CCA_score)
