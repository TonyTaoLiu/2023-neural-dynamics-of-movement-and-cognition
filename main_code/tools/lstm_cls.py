...
class LSTM(torch.nn.Module):
    "The LSTM network"
    def __init__(self, dtype, hidden_features=50, input_dims=10, output_dims = 8):
        super().__init__()
        torch.manual_seed(2147483647)
        self.hidden_features = hidden_features
        self.lstm1 = torch.nn.LSTM(input_dims, self.hidden_features,num_layers=2,batch_first=True)
        # self.lstm2 = torch.nn.LSTM(self.hidden_features, self.hidden_features,batch_first=True)
    ...
    def forward(self, x_in):
        ...
        #     outputs[i] = self.softmax(self.linear(h_t2))
        h_t, c_t = self.lstm1(x_in)
        outputs = self.softmax(self.linear(h_t[-1]))
        return outputs
...
class LSTMDecoder():
    ...
    def fit(self, x_train, y_train,
        ...
        for _ in range(epochs):
            for ind in shuffled_ind:
                ...
                labels = torch.from_numpy(y_train[ind, ...]).type(torch.cuda.LongTensor)
                # labels_onehot = torch.nn.functional.one_hot(labels,num_classes=8)
    ...
    def predict(self, x_test, y_test):
        ...
        for inputs, labels in zip(x_test_, y_test_):  # unravel the batches
            ...
            test_pred.append(np.argmax(output.cpu().detach().numpy()))
            test_labels.append(torch.unsqueeze(labels,dim=0).cpu().detach().numpy())

        pred = np.array(test_pred)
        label = np.concatenate(test_labels)
        self.score = np.mean(pred == label)

        return pred, label

if __name__ == "__main__":
    x = np.random.rand(400, 15,40)
    y = np.repeat(np.arange(8), 50)
    model = LSTMDecoder(40,8)
    model.fit(x, y)
